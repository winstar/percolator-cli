...
import { getOrCreateAssociatedTokenAccount, TOKEN_PROGRAM_ID, NATIVE_MINT, syncNative } from "@solana/spl-token";
import fs from "fs";
...
async function main() {
  ...
  // If low, wrap some SOL
  if (balance.value.uiAmount! < 2) {
    ...
    try {
      ...
      console.log("Wrap failed:", (err as any).message?.slice(0, 100));
    }
  }

  // Check updated balance
  const newBalance = await conn.getTokenAccountBalance(userAta.address);
  console.log("Updated ATA balance:", newBalance.value.uiAmount, "SOL\n");
